...
$(function() {
	if (smf_member_id > 0)
		$('.b_upshrink').each(function(index, el)
		{
			var catid = el.id.replace('b_upshrink_', '');
			new smc_Toggle({
				bToggleEnabled: true,
				bCurrentlyCollapsed: $('#b_upshrink_' + catid).data('collapsed'),
				aSwappableContainers: [
					'b_cat_' + catid + '_boards'
				],
				aSwapImages: [
					{
						sId: 'b_upshrink_' + catid,
						msgExpanded: '',
						msgCollapsed: '',
						cssCollapsed: 'b_toggle_down',
						cssExpanded: 'b_toggle_up'
					}
				],
				oThemeOptions: {
